...
class EditPurchaseDialog(QDialog):
    def __init__(self, purchase_data):
        super().__init__()
        self.setWindowTitle("✏️ Editar Factura")
        ...
        self.products_list = []
        self.detail_rows = []
        self.items_modified = False  # track si el usuario cambió items
    ...
    def setup_ui(self):
        ...
        self.state_combo.addItems(["pendiente", "recibido", "pagado", "vencido"])
        layout.addWidget(self._label("Estado"))
        layout.addWidget(self.state_combo)

        # --------------------------
        # Líneas de detalle
        # --------------------------
        layout.addWidget(self._label("📦 Detalle de productos"))

        # Estado de recepción — info
        self.reception_info = QLabel("")
        self.reception_info.setStyleSheet("color: #17A2B8; font-style: italic;")
        layout.addWidget(self.reception_info)
        ...
        self.btn_add_line.setStyleSheet("background-color: #28A745; color: white; font-weight: bold; padding: 4px 12px; border-radius: 4px;")
        self.btn_add_line.clicked.connect(self.add_detail_line)
        add_line_layout.addWidget(self.btn_add_line)
        ...
        layout.addWidget(self._label("Monto total"))
        ...
        # Botón PDF
        # --------------------------
        self.btn_pdf = QPushButton("📄 Reemplazar PDF (opcional)")
    ...
    # ----------------------------------------------------
    def fill_data(self):
        p = self.purchase_data

        self.invoice_input.setText(p["invoice_number"])
        self.amount_input.setText(str(p["amount"]))
        self.notes_input.setText(p.get("notes") or "")
        ...
        # fechas
        self.entry_date.setDate(QDate.fromString(p["entry_date"], "yyyy-MM-dd"))
        self.due_date.setDate(QDate.fromString(p["due_date"], "yyyy-MM-dd"))

        # estado
        status = p.get("status", "pendiente")
        if status in ["pendiente", "recibido", "pagado", "vencido"]:
            self.state_combo.setCurrentText(status)

        # recepción info
        received_at = p.get("received_at")
        if received_at:
            self.reception_info.setText(f"✅ Mercadería recibida el {str(received_at)[:10]}")

        # Si ya fue recibida o pagada, deshabilitar edición de items
        is_locked = status in ("recibido", "pagado")
        if is_locked:
            self.btn_add_line.setEnabled(False)
            self.reception_info.setText(
                self.reception_info.text() or "🔒 Items bloqueados (compra ya recibida/pagada)"
            )

        # Cargar items existentes (vienen del GET /purchases que hizo la vista)
        existing_items = p.get("items", [])
        for item in existing_items:
            product_name = item.get("product_name") or self._product_name(item["product_id"])
        ...
        self._refresh_items_table(allow_delete=not is_locked)
    ...
    def _refresh_items_table(self, allow_delete=True):
    ...
    def save(self):
        try:
            ...
            if not invoice or not amount:
                QMessageBox.warning(self, "Atención", "Número de factura y monto son obligatorios.")
                return
            ...
            data = {
                ...
                "status": self.state_combo.currentText(),
            }

            # Solo enviar items si fueron modificados
