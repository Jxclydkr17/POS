...
Tabla con filtros, paginación y botones de acción.
Mismo estilo visual oscuro del resto de la app.
"""
...
BTN_PRIMARY = """
    QPushButton {
        ...
        padding: 8px 14px; font-weight: 700; border-radius: 8px;
    }
...
class ProformasView(QWidget):
    ...
    def load_proformas(self):
        try:
            ...
            if resp.status_code != 200:
                show_toast("Error al cargar proformas", success=False, parent=self)
                return
            ...
            logger.error(f"Error cargando proformas: {e}")
            show_toast(f"Error de conexión", success=False, parent=self)
    ...
    def _fetch_customer_name(self, customer_id):
        try:
            ...
            if resp.status_code == 200:
                body = resp.json()
                # El endpoint retorna APIResponse: {"success":true, "data": {"name":"..."}}
                if isinstance(body, dict) and "data" in body:
                    return body["data"].get("name", "Cliente General")
                # Fallback por si retorna directamente el objeto
    ...
    def view_proforma(self, proforma_id):
        ...
        dlg.exec()
        self.load_proformas()  # refrescar por si cambió estado
    ...
    def download_pdf(self, proforma_id):
        try:
            resp = requests.get(
                f"{API_PROFORMAS}/{proforma_id}/pdf",
                headers=self._auth_headers(),
                timeout=15,
            )
            if resp.status_code == 200:
    ...
    def send_email(self, proforma_id):
        ...
        try:
            resp = requests.post(
                f"{API_PROFORMAS}/{proforma_id}/send-email",
                headers=self._auth_headers(),
                timeout=15,
            )
            if resp.status_code == 200:
    ...
    def void_proforma(self, proforma_id):
        confirm = QMessageBox.question(
            self, "Anular proforma",
            "¿Está seguro de anular esta proforma?\nEsta acción no se puede deshacer.",
            QMessageBox.Yes | QMessageBox.No,
        )
        if confirm != QMessageBox.Yes:
            return
