...
class PaymentDialog(QDialog):
    """Diálogo personalizado para registrar abonos con método de pago"""
    ...
    def get_values(self):
        try:
            ...
            gen_rep = self.chk_rep.isChecked()
            return amount, payment_method, gen_rep
        except ValueError:
...
_TABLE_STYLE = """
    ...
    QHeaderView::section {
        background-color: #3d3d3d; color: #E8E8E8;
...
class CustomerCreditView(QDialog):
    ...
    def setup_ui(self):
        ...
        self.progress_limit.setTextVisible(True)
        self.progress_limit.setFormat("Uso: %p%")
        self.progress_limit.setFixedHeight(20)
        self.progress_limit.setStyleSheet("""
            QProgressBar { background-color: #333; border-radius: 4px; text-align: center; color: white; font-size: 11px; }
        ...
        for btn, color in [
            (self.btn_view_sale, "#6b7280"),
            (self.btn_add_payment, "#28A745"),
            (self.btn_export_pdf, "#8b5cf6"),
            (self.btn_refresh, "#17A2B8"),
        ]:
            btn.setStyleSheet(f"""
                QPushButton {{ background-color: {color}; color: white; font-weight: bold;
                    padding: 7px; border-radius: 6px; min-width: 130px; }}
        ...
        self.btn_filter.clicked.connect(self._apply_date_filter)
        self.btn_filter.setStyleSheet("background-color: #555; color: white; padding: 5px 12px; border-radius: 4px;")
        filter_layout.addWidget(self.btn_filter)

        self.btn_clear_filter = QPushButton("Limpiar")
        self.btn_clear_filter.clicked.connect(self._clear_date_filter)
        self.btn_clear_filter.setStyleSheet("background-color: #444; color: #ccc; padding: 5px 12px; border-radius: 4px;")
        ...
        self.btn_mov_prev = QPushButton("◀")
        self.btn_mov_prev.clicked.connect(self._mov_prev)
        self.btn_mov_prev.setStyleSheet("background-color: #444; color: white; padding: 4px 10px; border-radius: 4px;")

        self.lbl_mov_page = QLabel("—")
        self.lbl_mov_page.setAlignment(Qt.AlignCenter)
        self.lbl_mov_page.setStyleSheet("color: #ccc; font-size: 12px; min-width: 120px;")

        self.btn_mov_next = QPushButton("▶")
        self.btn_mov_next.clicked.connect(self._mov_next)
        self.btn_mov_next.setStyleSheet("background-color: #444; color: white; padding: 4px 10px; border-radius: 4px;")

        pag_layout.addStretch()
        pag_layout.addWidget(self.btn_mov_prev)
        pag_layout.addWidget(self.lbl_mov_page)
        pag_layout.addWidget(self.btn_mov_next)
        pag_layout.addStretch()
        layout.addLayout(pag_layout)

        # ── Tabla de pendientes REP (headers renombrados) ──
        layout.addWidget(QLabel("📌 Comprobantes pendientes (para REP)"))

        self.pending_table = QTableWidget()
        self.pending_table.setColumnCount(7)
        self.pending_table.setHorizontalHeaderLabels([
            "Sel", "N° Venta", "N° Factura", "Fecha", "Total", "Pagado", "Pendiente"
        ])
        self.pending_table.setSelectionBehavior(QTableWidget.SelectRows)
        self.pending_table.setSelectionMode(QTableWidget.MultiSelection)
        self.pending_table.setStyleSheet(_TABLE_STYLE + _CHECKBOX_STYLE)
        layout.addWidget(self.pending_table)

        btn_refresh_pending = QPushButton("🔄 Refrescar pendientes")
        btn_refresh_pending.clicked.connect(self.load_pending_docs)
    ...
    def _aging_box(self, label, color):
        ...
        lbl.setStyleSheet(f"""
            background-color: #2a2a2a; border: 1px solid {color};
            border-radius: 6px; padding: 6px; color: {color};
    ...
    def load_credit_info(self):
        try:
            ...
            r = requests.get(
                f"{API_URL}/credits/{self.customer_id}",
                headers=self._auth(),
                params=params,
            )

            if r.status_code == 401:
                QMessageBox.warning(self, "Sesión expirada", "Debes iniciar sesión nuevamente.")
                return
            if r.status_code != 200:
                QMessageBox.warning(self, "Error", f"No se pudieron cargar los datos. Status: {r.status_code}")
                return
            ...
            if has_limit and credit_limit > 0:
                ...
                self.lbl_limit.setText("Límite: Ilimitado")
                self.progress_limit.setVisible(False)
            ...
            for row, m in enumerate(movements):
                ...
                if is_sale:
                    ...
                    if sale_id is not None:
                        id_item.setData(Qt.UserRole, int(sale_id))
            ...
            if self._mov_page > max_page:
                self._mov_page = max_page
    ...
    def load_pending_docs(self):
        try:
            r = requests.get(
                f"{API_URL}/ereps/pending-by-customer/{self.customer_id}",
                headers=self._auth()
            )
            if r.status_code != 200:
                QMessageBox.warning(self, "REP", f"No se pudo cargar pendientes:\n{r.text}")
                return
            ...
            QMessageBox.critical(self, "Error", f"Error cargando pendientes REP:\n{e}")
    ...
    def _selected_pending_einvoice_ids(self):
        ...
        for row in range(self.pending_table.rowCount()):
            ...
            if chk and chk.checkState() == Qt.Checked:
                try:
                    einv_id = int(self.pending_table.item(row, 2).text())
                    ids.append(einv_id)
                except Exception:
                    continue
        return ids
    ...
    # ==================================================================
    # REGISTRAR PAGO (sin cambios funcionales)
    # ==================================================================
    def register_payment(self):
        if not self.customer_id:
            QMessageBox.warning(self, "Sin cliente", "No hay cliente seleccionado.")
            return
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                r = requests.post(
                    f"{API_URL}/credits/{self.customer_id}/payments",
                ...
                if not payment_id:
                    QMessageBox.warning(self, "REP", "No se pudo obtener payment_id del servidor.")
                    return
                ...
                s = requests.post(
                    f"{API_URL}/ereps/suggest-allocations/{self.customer_id}",
                    json={"amount": amount, "electronic_invoice_ids": einv_ids},
                    headers=self._auth()
                )
                ...
                if not items:
                    QMessageBox.information(self, "REP", "No hay saldo pendiente aplicable.")
                    self.load_credit_info()
                    return

                resumen = "\n".join([
                    f"- Factura {it['electronic_invoice_id']}: ₡{it['amount_applied']:.2f}"
                    for it in items
                ])
                ...
                cr = requests.post(
                    f"{API_URL}/ereps/from-payment/{payment_id}",
    ...
    def export_pdf(self):
        if not self._last_data:
            QMessageBox.warning(self, "Sin datos", "Carga los datos primero.")
            return

        path, _ = QFileDialog.getSaveFileName(
            self, "Guardar estado de cuenta",
            f"estado_cuenta_{self.customer_name.replace(' ', '_')}.pdf",
            "PDF (*.pdf)"
        )
        if not path:
            return
        ...
        try:
            ...
            c.drawString(50, y, "Estado de Cuenta")
            y -= 25
            ...
            if movements:
                ...
                for m in movements:
                    ...
                    table_data.append([
                        str(m["id"]),
                        "Venta" if m["type"] == "sale" else "Abono",
                        m.get("payment_method", "N/A"),
                        f"{sign}₡{m['amount']:,.2f}",
                        m.get("created_at", ""),
                    ])
            ...
            # Abrir el PDF de forma cross-platform
    ...
    # ==================================================================
    # VER TICKET (cross-platform, reemplaza os.startfile)
    # ==================================================================
    def open_sale_pdf(self):
        ...
        if "Venta" not in tipo:
            QMessageBox.information(self, "No es una venta", "El movimiento seleccionado no corresponde a una venta.")
            return

        item = self.table.item(row, 0)
        sale_id = item.data(Qt.UserRole)
        if sale_id is None:
            QMessageBox.warning(self, "Error", "No se pudo determinar el ticket de esta venta.")
            return
    ...
    @staticmethod
    def _open_file(path):
        """Abrir archivo de forma cross-platform."""
    ...
    def on_row_selected(self):
        row = self.table.currentRow()
        if row == -1:
            self.btn_view_sale.setEnabled(False)
            return
        tipo = self.table.item(row, 1).text()
